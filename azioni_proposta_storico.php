...
if ($action == "read") {
    ...
    $query = "SELECT v.id as rif, d.nome as docente, ruolo, data as data_creazione FROM `docente` d JOIN (`partecipa` pa JOIN (`viaggio` v JOIN (`proposta` pr JOIN `consegue` c ON pr.id = c.rifProposta) ON v.id = pr.id) ON pa.rifViaggio = v.id) ON pa.rifDocente = d.id";

    // Aggiunta filtro di ricerca
    if (!empty($searchValue))
        $query .= " WHERE v.id LIKE :search OR d.nome LIKE :search OR ruolo LIKE :search";
    ...
    $totalRecordsQuery = "SELECT COUNT(*) FROM `consegue`";
    $totalRecords = $pdo->query($totalRecordsQuery)->fetchColumn();

    // Conteggio totale con filtro
    if (!empty($searchValue)) {
        $filteredRecordsQuery = "SELECT COUNT(*) FROM `docente` d JOIN (`partecipa` pa JOIN (`viaggio` v JOIN (`proposta` pr JOIN `consegue` c ON pr.id = c.rifProposta) ON v.id = pr.id) ON pa.rifViaggio = v.id) ON pa.rifDocente = d.id WHERE v.id LIKE :search OR d.nome LIKE :search OR `ruolo` LIKE :search";
    ...
    $rifP = $_POST["rif"];
    $rifV = $rifP;
    $stmt = $pdo->prepare("DELETE FROM `consegue` WHERE `rifProposta` = :rifP LIMIT 1");
    $stmt -> execute([":rifP" => $rifP]);
