...
if ($action == "read") {
    ...
    $columns = ["id", "docente", "ruolo", "data_creazione"];
    ...
    $query = "SELECT b.id, d.nome as docente, e.ruolo, e.data as data_creazione FROM `bozza` b LEFT JOIN (`effettua` e LEFT JOIN `docente` d ON e.rifDocente = d.id) ON e.rifBozza = b.id";
    ...
    if (!empty($searchValue)) {
        $filteredRecordsQuery = "SELECT COUNT(*) FROM `bozza` b LEFT JOIN (`effettua` e LEFT JOIN `docente` d ON e.rifDocente = d.id) ON e.rifBozza = b.id WHERE d.nome LIKE :search OR e.ruolo LIKE :search";
